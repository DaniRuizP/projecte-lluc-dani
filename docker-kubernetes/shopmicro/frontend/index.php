...
async function checkout() {
        ...
        // 1. Calculem el total a pagar abans d'enviar la comanda
        let totalToPay = 0;
        cart.forEach(item => totalToPay += item.price);

        try {
            // 2. Enviem la petició al backend per processar la compra
            ...
            // 3. Comprovem la resposta
            if(res.status === 'OK') { 
                // Buidem el carret local
                 cart.length = 0;
            
            // REDIRECCIÓ A LA PÀGINA D'ÈXIT
            window.location.href = 'success.php?total=' + totalToPay.toFixed(2);
            
        } else {
            // Si la base de dades diu que no hi ha estoc
